...
//	还有。。。。 各种必要的状态值，例如：是否生存.
	private boolean live=true; //生存状态 true 代表存在，false代表死亡 // 可以采用枚举值来定义这个(生存，死亡，隐身，无敌)						
	private long livingTime=0L;
	private int attack=0;
//	注明：当重新定义一个用于判定状态的变量，需要思考：1.初始化 2.值的改变 3.值的判定
	public ElementObj() {	//初始化三件套	
	}
...
//	 long ... aaa  不定长的 数组,可以向这个方法传输 N个 long类型的数据
	protected void changeClothes() {}//此处的传入该动作的第几个切片
	protected void move() {	}
